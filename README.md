...
| 화면 (특정 창) | `windows-capture` (WGC) — 가려진 창·HW 가속 게임 OK | `screen.mp4` |
...
2. (선택) **실행 파일** + **로그 폴더** — 둘 중 하나만 입력해도 다른 쪽을 자동 추론. 게임 자체가 디스크에 로그를 안 남기면 (AC/Anvil, Frostbite 등) 런처 로그가 잡힙니다
...
- **클로즈드 엔진 게임 로그**: 클로즈드 엔진 게임은 디스크 로깅이 없어 런처 로그만 잡힘. UE / Unity 게임은 `Saved/Logs` / `Player.log` 로 풍부
